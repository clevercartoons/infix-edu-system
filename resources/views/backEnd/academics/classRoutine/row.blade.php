<tr class="0" id="row_{{ $row }}"> 
    <td class="border-top-0"> 
        <div class="input-effect" >
            <select class="niceSelect  w-100 bb form-control subject_required selectSubject" data-subjet_row_id="{{ $row }}" name="routine[{{ $row }}][subject]"  id="subject_{{ $row }}" >
    ...
    <td class="border-top-0"> 
        <div class="row " id="teacher-div">
            <div class="col-lg-12 mt-30-md">
                <select class="niceSelect w-100 bb form-control selectTeacher" data-teacher_row_id="{{ $row }}" name="routine[{{ $row }}][teacher_id]" id="teacher_{{ $row }}">
    ...
    <td class="border-top-0">  
        <div class="row no-gutters input-right-icon">
            <div class="col">
                <div class="input-effect">
                    <input class="primary-input time start_time_required  form-control{{ @$errors->has('start_time') ? ' is-invalid' : '' }} selectStartTime" type="text" name="routine[{{ $row }}][start_time]" data-start_time_row_id = "{{ $row }}"  value="{{isset($routine)? $routine->start_time: ''}}" required id="start_time_{{ $row }}">
    ...
    <td class="border-top-0">   
        <div class="row no-gutters input-right-icon">
            <div class="col">
                <div class="input-effect">
                    <input class="primary-input time end_time_required  form-control{{ @$errors->has('end_time') ? ' is-invalid' : '' }} selectEndTime" type="text" name="routine[{{ $row }}][end_time]" data-end_time_row_id = "{{ $row }}"  value="{{isset($routine)? $routine->end_time: ''}}" required id="end_time_{{ $row }}">
    ...
    <td class="border-top-0">
        <div class="input-effect">
            <input type="checkbox" id="isBreak[{{ $row }}]" class="common-checkbox is_break_checkbox" data-row_id="{{ $row }}" value="1"
             ...
             >
                <label for="isBreak[{{ $row }}]">{{ __('common.Is Break') }}</label>
        </div>
    </td>
    ...
    <td class="border-top-0">   
        <div class="row">
            <div class="col-lg-12 mt-30-md">
                <select class="niceSelect w-100 bb form-control selectRoom" data-room_row_id="{{ $row }}" name="routine[{{ $row }}][room]" id="room_{{ $row }}">
